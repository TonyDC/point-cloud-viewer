import { DefaultLoadingManager, FileLoader, LoaderUtils, BufferGeometry, Float32BufferAttribute, PointsMaterial, VertexColors, Points } from 'three';
...
export default class PCDLoader {
    ...
    static parseHeader(data) {
        ...
        if (PCDheader.size !== null) {
            PCDheader.size = PCDheader.size[1].split(' ').map((x) => {
                return parseInt(x, 10);
            });
        }

        if (PCDheader.count !== null) {
            PCDheader.count = PCDheader.count[1].split(' ').map((x) => {
                return parseInt(x, 10);
            });
        } else {
            PCDheader.count = [];
            for (let i = 0, l = PCDheader.fields.length; i < l; i += 1) {
    ...
    parse(data, url) {
        const textData = LoaderUtils.decodeText(data);

        // parse header (always ascii format)
        const PCDheader = PCDLoader.parseHeader(textData);
